...
const Resume6 = (alldetails) => {
  ...
  return (
    <div className=" bg-white border border-gray-100 font-nunito mx-auto" style={{
      ...
      <div className='text-center font-bold mt-1 mb-2'>
        <p className='text-[27px] uppercase'>{contact?.firstName || ""} {contact?.lastName || ""}</p>
        {/* <p className='text-[12px]'>{contact?.jobTitle.name || "Human Resource Manager"}</p> */}
  ...
  <a
    href={linkedinUrl.startsWith("http") ? linkedinUrl : `https://${linkedinUrl}`}
      ...
      <div className='flex justify-center gap-1 text-[13px] p-1 '>
        <p>{contact?.address
          || ""}   , {contact?.city
          ...
          {experiences?.length > 0 ? (
            experiences.map((exp, index) => (
              <div key={exp._id || exp.id || index} className="">
                ...
                {(exp.jobTitle || exp.employer || exp.location) && ( <h3 className="font-semibold text-gray-900 break-words text-lg">
                  {exp.jobTitle || ""}{" "}
                ...
                <ul className="pt-2 ml-6 pb-3 text-gray-700 text-[15px] break-words
    [&_ol]:list-decimal 
    [&_ul]:list-disc 
    [&_li[data-list='bullet']]:list-disc 
    [&_li[data-list='ordered']]:list-decimal 
          ...
          {educations?.length > 0 ? (
            educations.map((edu, index) => (
              <div key={edu.id} className="">
                {(edu.schoolname || edu.degree || edu.location ) && (<h3 className="font-semibold text-gray-900 break-words text-lg">
                  {edu.
                    schoolname || ""}
                  <span className="text-gray-500 font-normal">
                    — {edu.degree || ""}
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            {skills.map((skill) => (
              <div key={skill._id || skill.id}>
                <p className="text-sm break-words text-gray-600 mb-1">
                  {skill.skill || ""}
                </p>
                {skill.level && (
                  <div className="h-[4px] w-full break-words bg-gray-300 rounded-full overflow-hidden">
                    <div
          ...
          {Array.isArray(finalize?.hobbiesAndInterests) &&
            finalize.hobbiesAndInterests.some(
              ...
              <div className='mt-1'>
                ...
                <div className='pt-1 pb-1  text-gray-700 text-[15px] break-words'>
          ...
          {Array.isArray(finalize?.websitesAndSocialMedia) &&
            finalize.websitesAndSocialMedia.some(
              ...
              <div className="mt-1">
                ...
                <div className="pt-1 pb-1 text-[15px] text-gray-700 break-words">
