...
const ResumePreviewModal = ({ show, onClose, template, onUse }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-[999] bg-black/10 backdrop-blur-[3px] flex items-center justify-center p-2 xs:p-3 sm:p-4 md:p-6">
  {/* Modal Container */}
  <motion.div
    initial={{ opacity: 0, scale: 0.95, y: 20 }}
    animate={{ opacity: 1, scale: 1, y: 0 }}
    transition={{ duration: 0.3, ease: "easeOut" }}
    className="relative bg-white w-full max-w-6xl h-[90vh] xs:h-[92vh] sm:h-[90vh] md:h-[88vh] lg:h-[85vh] rounded-lg xs:rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-gray-200/50"
  >
    {/* Header with Gradient */}
    <div className="shrink-0 bg-gradient-to-r from-white to-gray-50/80 border-b border-gray-100 px-3 xs:px-4 sm:px-6 lg:px-8 py-3 xs:py-4 sm:py-5 flex items-center justify-between">
      <div className="flex items-center gap-2 xs:gap-3">
        <div className="p-1.5 xs:p-2 bg-gradient-to-br from-[#c40116]/10 to-[#be0117]/10 rounded-lg">
          <svg
            className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6 text-[#c40116]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="max-w-[180px] xs:max-w-none">
          <h2 className="text-base xs:text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 truncate">
            {template.style || "Template Preview"}
          </h2>
          <p className="text-xs text-gray-500 truncate">
            Live Resume Preview • {template.category || "Professional Template"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 xs:gap-2">
        <button
          onClick={onClose}
          className="p-1.5 xs:p-2 sm:p-2.5 rounded-lg hover:bg-gray-100 transition-all duration-200 hover:scale-105 active:scale-95"
          aria-label="Close preview"
        >
          <X className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6 text-gray-500 hover:text-gray-700" />
        </button>
      </div>
    </div>

    {/* Scrollable Body */}
    <div className="flex-1 overflow-y-auto bg-gradient-to-br from-gray-50/50 to-white/30 p-3 xs:p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        {/* Resume Container with Shadow */}
        <div className="relative group">
          {/* Glow Effect - Only on larger screens */}
          <div className="absolute -inset-2 xs:-inset-3 sm:-inset-4 bg-gradient-to-r from-[#c40116]/5 to-[#be0117]/5 rounded-xl sm:rounded-2xl lg:rounded-3xl blur-lg sm:blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

          {/* Resume Image Container */}
          <div className="relative bg-white shadow-lg sm:shadow-xl rounded-lg sm:rounded-xl lg:rounded-2xl overflow-hidden border border-gray-200/50">
            {/* Image with Loading State */}
            <div className="relative w-full min-h-[300px] xs:min-h-[350px] sm:min-h-[400px] md:min-h-[500px] lg:min-h-[600px] flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100/50">
              <img
                src={template.image || template.img}
                alt={template.style || "Resume Template"}
                className="w-full h-auto object-contain transition-transform duration-500 group-hover:scale-[1.005]"
              ...
              {!template.image && !template.img && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="animate-spin rounded-full h-8 w-8 xs:h-10 xs:w-10 sm:h-12 sm:w-12 border-2 xs:border-3 border-[#c40116] border-t-transparent"></div>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Features List */}
        <div className="mt-6 sm:mt-8 bg-white rounded-lg xs:rounded-xl sm:rounded-2xl border border-gray-200 p-3 xs:p-4 sm:p-6">
          <h3 className="text-base xs:text-lg font-semibold text-gray-800 mb-3 xs:mb-4">
            Template Features
          </h3>
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-2 xs:gap-3 sm:gap-4">
            {template.features?.map((feature, index) => (
              <div
                key={index}
                className="flex items-start gap-2 xs:gap-3 p-2 xs:p-3 hover:bg-gray-50 rounded-lg transition-colors"
              >
                <div className="p-1 xs:p-1.5 bg-gradient-to-br from-[#c40116]/10 to-[#be0117]/10 rounded-lg shrink-0">
                  <Check className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-[#c40116]" />
                </div>
                <span className="text-xs xs:text-sm text-gray-700 break-words">
                  {feature}
                </span>
              </div>
            )) || (
              <>
                ...
                <div className="flex items-start gap-2 xs:gap-3 p-2 xs:p-3 hover:bg-gray-50 rounded-lg transition-colors col-span-1 xs:col-span-2 lg:col-span-1">
                  <div className="p-1 xs:p-1.5 bg-gradient-to-br from-[#c40116]/10 to-[#be0117]/10 rounded-lg shrink-0">
                    <Check className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-[#c40116]" />
                  </div>
                  <span className="text-xs xs:text-sm text-gray-700 break-words">
    ...
    {/* Footer */}
    <div className="shrink-0 border-t border-gray-100 bg-gradient-to-r from-white to-gray-50/80 px-3 xs:px-4 sm:px-6 lg:px-8 py-2 xs:py-3 sm:py-4">
      <div className="flex flex-col xs:flex-row items-center justify-between gap-2 xs:gap-3 text-xs text-gray-600">
        <div className="flex items-center gap-2 xs:gap-4 text-center xs:text-left">
          <span>© {new Date().getFullYear()} ARYU SmartCV</span>
          <span className="hidden xs:inline">•</span>
          <span className="hidden xs:inline">All rights reserved</span>
        </div>
        <div className="text-xs text-gray-600">
          Preview mode • Scroll to view details
        </div>
      </div>
    </div>
  </motion.div>
