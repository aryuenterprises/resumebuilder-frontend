...
const TemplateTwelve: React.FC<TemplateTwelveProps> = ({
  ...
  const buildCSS = useCallback((fontFamily: string) => {
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .t12-resume p { margin: 0 0 4px 0 !important; padding: 0 !important; line-height: 1.5 !important; }
    ...
    @media print {
      *, *::before, *::after {
      ...
      .project-link, .header-meta-item a { color: #111 !important; text-decoration: none !important; }
  ...
  const generateHTML = useCallback(
    (forPDF = false, pageBreakIds: string[] = []): string => {
      ...
      <div class="section-block" data-block-id="t12-summary">
        <div class="section-title">Profile</div>
        <div class="summary-text" data-block-id="t12-summary-content">${rich(summary)}</div>
      </div>
    `
            : "",
        ...
        ${educations
          .map((edu: any, i: number) => {
            ...
            const dateStr = [
              edu.startDate || "",
              edu.startDate ? "\n–\n" : "",
              edu.endDate ? edu.endDate : edu.isCurrentlyStudying ? "Present" : "",
            ].join("");
            return `
            <div class="entry-block" data-block-id="t12-edu-${i}">
              <div class="entry-left">
...
<body style="margin:0;padding:0;background:white;">
  ...
  const buildPDFPagesHTML = useCallback(
    ...
    <div class="pdf-page" style="position:relative;width:${A4_W}px;height:${A4_H}px;overflow:hidden;background:white;${!isLastPage ? "page-break-after:always;break-after:page;" : ""}">
      <div style="position:absolute;top:${MARGIN}px;left:${MARGIN}px;width:${A4_W - MARGIN * 2}px;height:${clipH}px;overflow:hidden;">
        <div style="position:absolute;top:${-contentOffsetY}px;left:0;width:${A4_W - MARGIN * 2}px;">
          ${resumeSnapshot}
        </div>
      </div>
    </div>`;
...
<head>
  ...
  <style>
    ...
    .t12-resume { width: ${A4_W - MARGIN * 2}px !important; padding-top: 0 !important; padding-bottom: 0 !important; padding-left: 0 !important; padding-right: 0 !important; margin: 0 !important; }
        ...
        const doMeasure = () => {
          ...
          Array.from(resume.querySelectorAll<HTMLElement>("*")).forEach(
            (el) => {
              ...
              // Skip the narrow date column — rides along with its row
              if (el.matches(".entry-left, .entry-date")) {
                consumed.add(el);
                return;
              }
          ...
          resume
            .querySelectorAll<HTMLElement>(
              ".header-name, .header-jobtitle, .header-divider, .header-meta-row, .project-link, .header-meta-item a",
            )
            .forEach((el) => {
              if (consumed.has(el)) return;
              pushAtomic(el, false);
