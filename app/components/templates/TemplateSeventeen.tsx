...
const TemplateSeventeen: React.FC<TemplateSeventeenProps> = ({
  ...
  // ✅ NEW: Use <link> tag instead of @import for faster font loading
...
<body style="margin:0;padding:0;background:white;">
  ...
  // ✅ NEW: PDF builder (clip/shift — matches preview exactly)
...
<body style="margin:0;padding:0;background:white;">
  ...
  // ✅ NEW: Memoized CSS & font link for measurement
  ...
  // ── Page splitter (reuses iframe, smart font wait, line-level packer) ───────
  const splitIntoPages = useCallback(
    (fullHtml: string): Promise<string[]> => {
      return new Promise((resolve) => {
        const parser = new DOMParser();
...
<body>${resumeSnapshot}</body></html>`);
        ...
        const doMeasure = () => {
          ...
          const CHAINED_KEEP_SELECTOR = [".t17-entry-subtitle"].join(", ");
          ...
          resume
            .querySelectorAll<HTMLElement>(
              ".t17-header-name, .t17-header-jobtitle, .t17-header-divider, .t17-header-meta-row, .t17-header-meta-item, .t17-entry-title, .t17-entry-date",
            )
            .forEach((el) => {
              if (consumed.has(el)) return;
              pushAtomic(el, el.classList.contains("t17-entry-title"));
              consumed.add(el);
            });
