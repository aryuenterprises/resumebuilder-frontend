...
//   // ── Page splitter (line-level engine, matching TemplateOne) ────────────────
//   const CSS_FOR_MEASURE = buildCSS(activeFontFamily);
...
//           const CHAINED_KEEP_SELECTOR = [".t15-entry-sub"].join(", ");
...
//           // Fallback: name/job-title/contact-row (single leaves, no p/li).
//           resume
//             .querySelectorAll<HTMLElement>(".t15-name, .t15-jobtitle, .t15-contact-row")
//             .forEach((el) => {
//               if (consumed.has(el)) return;
//               pushAtomic(el, false);
//               consumed.add(el);
...
//   // started splitIntoPages call's result is ever applied. Without this, a
//   // slower-resolving call for stale (shorter) content can overwrite a
//   // faster-resolving call for the latest content, causing freshly typed
//   // text to visually clip or vanish until a later edit happens to resolve
//   // last.
//   useEffect(() => {
//     if (!htmlContent) return;
...
const PLAYFAIR_LINK =
  '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700;800&display=swap"/>';
...
<head>
  ...
  <style>
    ...
    .t15-resume { width: ${A4_W}px !important; padding-top: 0 !important; padding-bottom: 0 !important; padding-left: ${MARGIN}px !important; padding-right: ${MARGIN}px !important; margin: 0 !important; }
        ...
        const doMeasure = () => {
          ...
          resume
            .querySelectorAll<HTMLElement>(
              ".t15-name, .t15-jobtitle, .t15-contact-row, .t15-entry-title, .t15-entry-date, .t15-education-grade, .t15-project-link, .t15-header-link",
            )
            .forEach((el) => {
              if (consumed.has(el)) return;
              pushAtomic(
