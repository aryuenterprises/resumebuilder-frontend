...
const TemplateOne: React.FC<TemplateOneProps> = ({
  ...
  const buildCSS = useCallback(
    ...
    .t1-resume-body p { margin: 0 !important; padding: 0 !important; line-height: 1.5 !important; }
  ...
  // ── Unified Reusable Resume Tree ────────────────────────────────────────────
  const renderResumeContent = () => (
    <div className="t1-resume-body">
      {/* Header */}
      <div className="t1-contact-info" data-block-id="header">
        <div className="t1-name">
          {contact?.firstName || ""} {contact?.lastName || ""}
        </div>
        <div className="t1-job-title">
          {typeof contact?.jobTitle === "string"
            ? contact.jobTitle
            : (contact?.jobTitle as any)?.name || ""}
        </div>
        {addressParts.length > 0 && (
          <div className="t1-address">{addressParts.join(", ")}</div>
        )}
        <div className="t1-contact-details">
          {contact?.email && <span>{contact.email}</span>}
          {contact?.phone && <span>{contact.phone}</span>}
          {formattedDob && <span>{formattedDob}</span>}
        </div>
      ...
      {/* Summary */}
      {summary?.trim() && (
        <div className="t1-section-content" data-block-id="summary">
          <div className="t1-section-title">Summary</div>
          <div
            className="t1-item-content t1-summary-text"
      ...
      {experiences.length > 0 && (
        <div className="t1-section-content" data-block-id="exp-section">
          <div className="t1-section-title">Experience</div>
          {experiences.map((exp: any, i: number) => {
            ...
            return (
              <div
                key={i}
                className="t1-experience-item"
                data-block-id={`exp-${i}`}
                style={{ marginBottom: "16px" }}
              >
                <div className="t1-item-header">
                  <div className="t1-item-title-container">
                    <div className="t1-item-title">{exp.jobTitle || ""}</div>
                    <div className="t1-item-subtitle">
                      {exp.employer || ""}
                      {exp.location ? ` — ${exp.location}` : ""}
                    </div>
                  </div>
                  <div className="t1-item-date t1-experience-date">
                    {s} - {e}
                  </div>
                </div>
                {exp.text && (
                  <div
                    className="t1-item-content t1-experience-description"
                    dangerouslySetInnerHTML={{
      ...
      {/* Projects */}
      {projects.length > 0 && (
        <div className="t1-section-content" data-block-id="proj-section">
          <div className="t1-section-title">Projects</div>
          {projects.map((p: any, i: number) => (
            <div
              key={i}
              className="t1-project-item"
              data-block-id={`proj-${i}`}
            >
              <div className="t1-project-header">
                <div className="t1-project-title">{p.title || ""}</div>
                {(p.liveUrl || p.githubUrl) && (
                  <div className="t1-project-links">
                    {p.liveUrl && (
                      <a
                        href={href(p.liveUrl)}
                        className="t1-project-link"
                        target="_blank"
                        rel="noreferrer"
                      >
                        Live Demo
              ...
              {p.techStack?.length > 0 && (
                <div className="t1-project-tech-stack">
                  <strong>Tech:</strong> {p.techStack.join(" , ")}
                </div>
              )}
              {p.description && (
                <div
                  className="t1-item-content t1-project-description"
                  dangerouslySetInnerHTML={{
      ...
      {/* Education */}
      {educations.length > 0 && (
        <div className="t1-section-content" data-block-id="edu-section">
          ...
          {educations.map((edu: any, i: number) => {
            ...
            return (
              <div
                key={i}
                className="t1-education-item"
                data-block-id={`edu-${i}`}
                style={{ marginBottom: "16px" }}
              >
                <div className="t1-item-header">
                  <div className="t1-item-title-container">
                    <div className="t1-item-title">{edu.degree || ""}</div>
                    <div className="t1-item-subtitle">
                      {edu.schoolname ? <span>{edu.schoolname}</span> : ""}
                      {edu.schoolname && edu.location ? " — " : ""}
                      {edu.location ? <span>{edu.location}</span> : ""}
                      {(edu.schoolname || edu.location) && grade ? " • " : ""}
                      {grade ? (
                        <span className="t1-education-grade">{grade}</span>
                      ) : (
                        ""
                      )}
                    </div>
                  ...
                  {dateStr && (
                    <div className="t1-item-date t1-education-date">
                      {dateStr}
                    </div>
                  )}
                </div>
                {edu.text && (
                  <div
                    className="t1-item-content t1-education-description"
                    dangerouslySetInnerHTML={{
      ...
      {/* Skills */}
      {hasSkills && (
        <div className="t1-section-content" data-block-id="skills-section">
          <div className="t1-section-title">Skills</div>
          <div
            className="t1-skills-content"
            data-block-id="skills-content"
            dangerouslySetInnerHTML={{
              __html: cleanQuillHTML(skills),
            }}
          />
        </div>
      )}
      ...
      {!Array.isArray(finalize) &&
        ...
        finalize.customSection.some(
          ...
          <div className="t1-section-content">
            {finalize.customSection
              .filter((s: any) => s?.name?.trim() || s?.description?.trim())
              .map((s: any, i: number) => (
                <div
                  key={i}
                  className="t1-custom-section"
                  data-block-id={`custom-${i}`}
                >
                  {s.name && <div className="t1-section-title">{s.name}</div>}
                  {s.description && (
                    <div
                      className="t1-item-content t1-custom-section-content"
                      dangerouslySetInnerHTML={{
                        __html: cleanQuillHTML(s.description),
                      }}
                    />
                  )}
                </div>
  ...
  const calculatePagination = useCallback(() => {
    ...
    const resumeEl = el.querySelector<HTMLElement>(".t1-resume-body");
    if (!resumeEl) return;

    // Force layout flush so bounding rects are 100% accurate
    void resumeEl.offsetHeight;

    const resumeRect = resumeEl.getBoundingClientRect();
    const totalH = resumeEl.scrollHeight;

    // Fast path: if all content easily fits inside 1 single A4 page
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      // Has this block crossed beyond the current page boundary?
      if (relativeBottom - currentStart > PAGE_CONTENT_H) {
        let breakIndex = i;

        // Orphan Header Prevention:
        // If the item causing the break is right after a section title,
        // pull the section title onto the new page together with this item.
        if (
          breakIndex > 0 &&
          blocks[breakIndex - 1].classList.contains("t1-section-title") &&
          blocks[breakIndex - 1].getBoundingClientRect().top - resumeRect.top >= currentStart
        ) {
          breakIndex--;
        }

        const targetTop = blocks[breakIndex].getBoundingClientRect().top - resumeRect.top;
        currentStart = targetTop > currentStart ? targetTop : relativeTop;
        offsets.push(currentStart);
        i = breakIndex; // continue packing from this break position
      }
    }

    // Safety fallback: if total height still exceeds page boundaries
  ...
  // Recalculate on edits & when Google Fonts finish rendering
  useEffect(() => {
  ...
  const handleDownload = async (): Promise<void> => {
    ...
    try {
      const resumeSnapshot =
        measureRef.current.querySelector(".t1-resume-body")?.outerHTML || "";
      const fontUrl = getFontImport(activeFontFamily);
      const cssStyles = buildCSS(activeFontFamily);
      const totalH =
        measureRef.current.querySelector(".t1-resume-body")?.scrollHeight || A4_H;
...
<body style="margin:0;padding:0;background:white;">
  ...
  return (
    <>
      ...
      {/* ── Offscreen In-Memory Measure Container (Never unmounted) ─────── */}
      <div
        ref={measureRef}
        aria-hidden="true"
        style={{
      ...
      >
        {pageOffsets.map((offsetY, idx) => {
          const totalH =
            measureRef.current?.querySelector(".t1-resume-body")?.scrollHeight ||
            A4_H;
          const nextStart = pageOffsets[idx + 1] ?? totalH;
          const clipH = nextStart - offsetY;
