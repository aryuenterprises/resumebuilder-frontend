...
${portfolioUrl ? `<a href="${href(portfolioUrl)}" class="link-item" target="_blank">${formatSocialLink(portfolioUrl, "portfolio")}</a>` : ""}          </div>
        </div>`;
...
<body>
  ...
  setIsDownloading(true);
  try {
    // No browser-computed pageBreakIds — WeasyPrint's text layout differs
    // from the browser's, so forced breaks at browser offsets can create
    // mismatched or blank pages. Let WeasyPrint paginate the natural flow
    // using the CSS avoid-rules in buildCSS.
