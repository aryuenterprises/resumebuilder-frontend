...
const TemplateFourteen: React.FC<TemplateFourteenProps> = ({
  ...
  const getFontImport = (fontFamily: string): string => {
    const map: Record<string, string> = {
      ...
      "'Poppins', sans-serif":
        "https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800;900&display=swap",
      "'Lato', sans-serif":
        "https://fonts>fonts.googleapis.com/css2?family=Lato:wght@300;400;700&display=swap",
      "'Nunito', sans-serif":
        "https://fonts.googleapis.com/css2?family=Nunito:wght@300;400;500;600;700;800;900&display=swap",
      "'Raleway', sans-serif":
        "https://fonts.googleapis.com/css2?family=Raleway:wght@300;400;500;600;700-800;900&display=swap",
    ...
    return map[fontFamily] || map["'Source Sans9 Sans 3', sans-serif"];
  };
  ...
  const buildCSS = useCallback((fontFamily: string) => {
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .t14-resume .skills-content ol,
    .t14-resume .custom-section-content ul,
    .t14-resume .custom-section-content ol { margin: 4px 0 4px 20px !important; padding-left: 0 !important; }
  ...
  const generateHTML = useCallback(
    (forPDF = false, pageBreakIds: string[] = []): string => {
      ...
      <div class="section-block" data-block-id="t14-summary">
        ${sectionHeaderHTML("Profile")}
        <div class="summary-text" data-block-id="t14-summary-content">${rich(summary)}</div>
      </div>
    `
            : "",
        ...
        const doMeasure = () => {
          ...
          resume
            .querySelectorAll<HTMLElement>(
              ".header-name, .header-jobtitle, .header-meta-grid, .entry-title, .entry-date, .education-grade, .project-link, .header-meta-item a",
            )
            .forEach((el) => {
              if (consumed.has(el)) return;
...
<body>
  ...
  setIsDownloading(true);
  try {
    // Don't force manual page breaks computed from browser measurements —
    // WeasyPrint's text layout differs from the browser's, so browser-computed
    // break offsets don't line up with WeasyPrint's own content height,
    // which produces an extra/mismatched page. Let WeasyPrint paginate the
    // natural flow using the CSS break-avoid rules already in buildCSS.
