...
export const cleanQuillHTML = (html: string): string => {
  ...
  for (const element of elements) {
    if (element.tag === "ol" || element.tag === "ul") {
      ...
      if (hasOrderedItems && hasBulletItems) {
        ...
        while ((liMatch = liRegex.exec(element.content)) !== null) {
          items.push({ type: liMatch[1], content: liMatch[2] });
        }

        let currentType = "";
        let currentItems: string[] = [];
        let orderedCounter = 1; // tracks the next ordered number

        const flushList = () => {
          if (currentItems.length === 0) return;
          if (currentType === "ordered") {
            // Preserve continuous numbering across interruptions
            const startAttr =
              orderedCounter > 1
                ? ` start="${orderedCounter - currentItems.length}"`
                : "";
            result += `<ol class="resume-list"${startAttr}>`;
        ...
        for (const item of items) {
          ...
          if (item.type === "ordered") orderedCounter++;
        }
        flushList();
  ...
  result = result.replace(
    /<a /g,
    '<a style="color: #0077b5; text-decoration: underline; cursor: pointer;" ',
  );
