...
//   FiEye,
// } from "react-icons/fi";
// import { HiOutlineBadgeCheck, HiOutlineReceiptRefund } from "react-icons/hi";
...
// // ---------------------------------------------------------------------------
// // Plan configuration — drives plan badges, tier styling, and the feature
// // gating used to lock/unlock resume editing across the dashboard.
// // ---------------------------------------------------------------------------
...
// const normalizePlanKey = (planName?: string | null): PlanKey => {
//   if (!planName) return "no_plan";
//   const key = planName.toLowerCase().trim().replace(/\s+/g, "_");
//   if (key in PLAN_CONFIG) return key as PlanKey;
...
//   // ---------------------------------------------------------------------
//   // Plan-derived state — single source of truth for whether resume
//   // editing should be locked, and which PLAN_CONFIG entry is "active".
//   // ---------------------------------------------------------------------
...
//   const isNoPlanState = !usersCurrentPlan;

//   const isEditLocked = isNoPlanState || isExpiredState;

//   const activePlanKey: PlanKey = isEditLocked
//     ? "no_plan"
//     : normalizePlanKey(usersCurrentPlan?.current_plan);

//   const activePlanConfig = PLAN_CONFIG[activePlanKey];
...
//         if (viewingResume && String(viewingResume.id) === String(id)) {
...
//       showBillingHistory || showLogoutModal || viewingResume
//         ? "hidden"
//         : "auto";
//     return () => {
...
//   // Navigates into the resume editor for a given resume — gated on plan status.
//   const handleEditResume = (item: ResumeItem) => {
//     if (isEditLocked) {
//       toast.error("Renew your plan to edit this resume.");
//       return;
//     }
...
//       value: statsData?.total_resumes_created ?? totalResumes,
//       icon: FiFileText,
//     },
//     {
...
//           >
//             <span className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-indigo-50 rounded-full text-indigo-600 text-[10px] sm:text-xs font-semibold mb-3 sm:mb-4 tracking-wide uppercase">
...
//                 className="flex items-center justify-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-indigo-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-600/20 transition-all duration-300 cursor-pointer"
//               >
...
//                 className="bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-gray-100 hover:border-indigo-100 transition-colors"
//               >
//                 <div className="flex items-center justify-between">
//                   <div>
//                     <p className="text-xs text-gray-500">{stat.label}</p>
//                     <p
//                       className={`text-lg lg:text-2xl font-bold mt-0.5 ${
//                         stat.locked ? "text-gray-500" : "text-indigo-600"
//                       }`}
//                     >
//                       {stat.value}
//                     </p>
...
//                       stat.locked ? "bg-gray-100" : "bg-indigo-50"
//                     }`}
//                   >
//                     <stat.icon
//                       className={`w-5 h-5 ${
...
//                 <div className="relative h-20 sm:h-24 bg-indigo-600">
//                   <div className="absolute -bottom-8 sm:-bottom-9 left-4 sm:left-6 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white border-4 border-white shadow-md flex items-center justify-center">
//                     <span className="text-indigo-600 text-base sm:text-lg font-bold">
...
//                       <div
//                         key={idx}
//                         className="flex items-center gap-2 sm:gap-3 p-1.5 sm:p-2 rounded-lg sm:rounded-xl hover:bg-indigo-50/60 transition-colors"
//                       >
//                         <div className="p-1.5 sm:p-2 bg-indigo-50 rounded-lg">
//                           <item.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600" />
//                         </div>
...
//                     className="w-full mt-4 sm:mt-6 px-3 sm:px-4 py-2.5 sm:py-3 bg-indigo-50 text-indigo-600 font-medium rounded-lg sm:rounded-xl hover:bg-indigo-100 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer text-xs sm:text-sm"
//                   >
...
//                               <span className="px-1.5 sm:px-2 py-0.5 bg-red-500/20 text-red-100 text-[10px] sm:text-xs font-semibold rounded-full">
//                                 EXPIRED
//                               </span>
...
//                                 You can still view your existing resumes. To
//                                 edit them or create new ones, please subscribe
//                                 to a new plan.
//                               </p>
//                             </div>
//                           </div>
...
//                             className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 bg-indigo-600 text-white font-medium rounded-lg sm:rounded-xl hover:bg-indigo-700 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer text-xs sm:text-sm"
//                           >
...
//                     <>
//                       <div
//                         className={`bg-gradient-to-r ${activePlanConfig.color} p-4 sm:p-6`}
//                       >
//                         <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
//                           <div>
//                             <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
//                               <span className="px-1.5 sm:px-2 py-0.5 bg-white/20 text-white text-[10px] sm:text-xs font-semibold rounded-full">
//                                 ACTIVE
//                               </span>
//                               <HiOutlineBadgeCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/80" />
//                             </div>
//                             <h3 className="text-lg sm:text-xl font-bold text-white mb-0.5 sm:mb-1">
...
//                           usersCurrentPlan?.current_plan === "Premium") && (
//                           <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl border border-gray-100">
...
//                     <div className="inline-flex p-3 sm:p-4 bg-white/15 rounded-full mb-3 sm:mb-4">
...
//                     </p>
//                   </div>
//                   <div className="p-4 sm:p-6 text-center flex items-center justify-center gap-3 sm:gap-4">
//                     <motion.button
//                       whileHover={{ scale: 1.02 }}
//                       whileTap={{ scale: 0.98 }}
//                       onClick={() => router.push("/choose-plan")}
...
//                   <FiFileText className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600" />{" "}
//                   Your Resumes
//                 </h3>
...
//                   </p>
//                   <span className="px-1.5 sm:px-2 py-0.5 bg-indigo-50 rounded-full text-indigo-600 text-[10px] sm:text-xs font-semibold">
...
//             {/* Subscription required banner — resumes stay viewable, editing is locked */}
//             {isExpiredState && (
//               <motion.div
//                 initial={{ opacity: 0, y: -10 }}
//                 animate={{ opacity: 1, y: 0 }}
//                 className="mb-4 p-3 sm:p-4 bg-amber-50 border border-amber-200 rounded-lg sm:rounded-xl flex flex-col sm:flex-row sm:items-center items-start gap-2 sm:gap-3"
//               >
//                 <div className="p-1.5 sm:p-2 bg-amber-100 rounded-lg shrink-0">
...
//                   </p>
//                   <p className="text-amber-700 text-[11px] sm:text-xs">
//                     Your resumes are still visible. Renew your subscription to
...
//                       {/* Locked ribbon — resume stays viewable, only editing is restricted */}
//                       {isEditLocked && (
//                         <div className="absolute top-2 left-2 z-10 flex items-center gap-1 px-2 py-1 rounded-full bg-amber-500 text-white text-[10px] sm:text-[11px] font-semibold shadow-sm">
...
//                       <motion.div
//                         className={`absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/45 to-transparent ${!isMobile ? "opacity-0 group-hover:opacity-100" : ""} transition-all duration-300`}
//                         initial={{ opacity: 0 }}
...
//                               onClick={(e) => {
//                                 e.stopPropagation();
//                                 setViewingResume(item);
//                               }}
//                               className="bg-white rounded-full p-1.5 sm:p-2.5 hover:bg-indigo-50 transition-all duration-300 shadow-lg cursor-pointer group/btn"
//                               title="View resume"
//                             >
//                               <FiEye className="h-3 w-3 sm:h-4 sm:w-4 text-indigo-600 group-hover/btn:scale-110 transition-transform" />
//                             </motion.button>
//                             <motion.button
//                               whileHover={{ scale: isEditLocked ? 1 : 1.1 }}
//                               whileTap={{ scale: isEditLocked ? 1 : 0.95 }}
//                               onClick={(e) => {
//                                 e.stopPropagation();
//                                 handleEditResume(item);
//                               }}
//                               className={`rounded-full p-1.5 sm:p-2.5 transition-all duration-300 shadow-lg cursor-pointer group/btn ${
//                                 isEditLocked
//                                   ? "bg-white/70"
...
//                               title={
//                                 isEditLocked
//                                   ? "Renew your plan to edit"
//                                   : "Edit resume"
//                               }
//                             >
//                               {isEditLocked ? (
//                                 <FiLock className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400" />
//                               ) : (
//                                 <FiEdit2 className="h-3 w-3 sm:h-4 sm:w-4 text-indigo-600 group-hover/btn:scale-110 transition-transform" />
//                               )}
//                             </motion.button>
//                             <motion.button
//                               whileHover={{ scale: 1.1 }}
...
//                                 animate={{ opacity: 1, scale: 1 }}
//                                 className="absolute right-2 top-12 z-20 w-40 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5"
//                               >
//                                 <button
//                                   onClick={(e) => {
//                                     e.stopPropagation();
...
//                                   }}
//                                   className="w-full px-3 py-2 text-left text-xs text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors flex items-center gap-2"
//                                 >
...
//                     <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 mx-auto mb-4 sm:mb-6 bg-indigo-50 rounded-2xl flex items-center justify-center">
//                       <FiFileText className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-indigo-600" />
//                     </div>
//                     <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 mb-1 sm:mb-2">
//                       No Resumes Found
//                     </h3>
//                     <p className="text-gray-500 text-xs sm:text-sm mb-4 sm:mb-6">
//                       {isNoPlanState || isExpiredState
//                         ? "Subscribe to a plan to create your first resume"
//                         : "Create your first resume to get started"}
//                     </p>
//                     <button
//                       onClick={() => router.push("/choose-template")}
//                       className="px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-all text-xs sm:text-sm cursor-pointer bg-indigo-600 text-white hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-600/20"
//                     >
//                       Create Resume
//                     </button>
//                   </div>
...
//             >
//               {/* Header */}
//               <div className="bg-indigo-600 p-4 sm:p-5 flex-shrink-0">
//                 <div className="flex items-center justify-between gap-3">
//                   <div className="flex items-center gap-2 sm:gap-3 min-w-0">
//                     <div className="p-1.5 sm:p-2 bg-white/15 rounded-lg sm:rounded-xl shrink-0">
...
//                         {viewingResume.resume_title || "Resume Preview"}
//                       </h2>
//                       <p className="text-white/80 text-[11px] sm:text-xs">
//                         Template: {viewingResume.template?.name ||
//                           viewingResume.template?.id}
//                       </p>
//                     </div>
//                   </div>
//                   <button
//                     onClick={() => setViewingResume(null)}
//                     className="p-1.5 sm:p-2 hover:bg-white/20 rounded-lg transition cursor-pointer shrink-0"
//                   >
...
//                 <div
//                   className="mx-auto bg-white shadow-sm border border-gray-100 rounded-lg overflow-hidden"
//                   style={{ width: "min(794px, 100%)" }}
//                 >
//                   {(() => {
//                     const ComponentToRender = viewingResume.component;
//                     return (
//                       <ComponentToRender alldata={viewingResume.resume_data} />
//                     );
//                   })()}
//                 </div>
//               </div>
...
//               {/* Footer actions */}
//               <div className="p-3 sm:p-4 border-t border-gray-100 flex flex-col sm:flex-row gap-2 sm:gap-3 flex-shrink-0">
//                 {isEditLocked && (
//                   <span className="flex-1 flex items-center gap-1.5 text-amber-700 text-[11px] sm:text-xs bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
//                     <FiLock className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
//                     Renew your plan to edit this resume.
//                   </span>
//                 )}
//                 <motion.button
//                   whileHover={{ scale: isEditLocked ? 1 : 1.02 }}
//                   whileTap={{ scale: isEditLocked ? 1 : 0.98 }}
//                   onClick={() => handleEditResume(viewingResume)}
//                   className={`px-4 sm:px-5 py-2 sm:py-2.5 font-medium rounded-lg sm:rounded-xl transition-all duration-300 flex items-center justify-center gap-2 text-xs sm:text-sm ${
//                     isEditLocked
...
//             >
//               {/* Fixed Header */}
//               <div className="bg-indigo-600 p-4 sm:p-6 flex-shrink-0">
//                 <div className="flex items-center justify-between">
//                   <div className="flex items-center gap-2 sm:gap-3">
//                     <div className="p-1.5 sm:p-2 bg-white/15 rounded-lg sm:rounded-xl">
...
//                         </div>
//                         <div className="w-7 h-7 sm:w-8 sm:h-10 bg-indigo-50 rounded-lg sm:rounded-xl flex items-center justify-center">
//                           <HiOutlineBadgeCheck className="w-3.5 h-3.5 sm:w-4 sm:h-5 text-indigo-600" />
//                         </div>
//                       </div>
//                     </motion.div>
//                   </div>
...
//                 <div className="inline-flex p-2.5 sm:p-3 bg-white/15 rounded-xl sm:rounded-2xl mb-2 sm:mb-3">
