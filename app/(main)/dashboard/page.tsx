...
// ---- A4 constants used to responsively scale the "view resume" modal ----
const A4_WIDTH = 794;
const A4_HEIGHT = 1123;

// The `template` object that comes back from the /dashboard API only has
// { id, name, thumbnail, tier } — it doesn't carry the `pic` flag that lives
// on the template definitions in app/data (templateData). ContactForm reads
// `chosenResumeDetails?.pic === "true"` to decide whether to show the photo
// upload UI, so storing the raw API template object as "chosenTemplate"
// silently drops photo upload for templates that support it. This looks
// up the matching entry in templateData and merges its `pic` value in
// before anything gets written to localStorage.
const enrichTemplateForStorage = (apiTemplate: Resume["template"]) => {
  const fullTemplateDefinition = templateData.find(
    (t) => t?.id == apiTemplate?.id,
  );
  return {
    ...apiTemplate,
    pic: fullTemplateDefinition?.pic,
  };
};
...
const DashboardPage = () => {
  ...
  // Page loading state (initial dashboard fetch)
  const [isPageLoading, setIsPageLoading] = useState(true);

  // Resume delete-in-progress state, keyed by resume id, so only that card
  // shows a spinner instead of a global blocking loader.
  ...
  // Lock page scroll while the view-resume modal is open too
  useEffect(() => {
    document.body.style.overflow = viewingResume ? "hidden" : "auto";
    return () => {
      document.body.style.overflow = "auto";
    };
  }, [viewingResume]);

  // Responsive scaling for the view-resume modal, based on available width
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      const nextScale = Math.min(containerWidth / A4_WIDTH, 1);
  ...
  // Shared edit-navigation logic (used by the desktop hover overlay, the
  // mobile action menu, and the view modal's Edit shortcut) so the
  // chosenTemplate/pic fix and the editingResumeIdAndData payload only
  // have to be written in one place.
  const handleEditResume = (item: ResumeItem) => {
    setLocalStorage("chosenTemplate", enrichTemplateForStorage(item.template));
  ...
  // Reuses the same "chosenTemplate" / "editingResumeIdAndData" handoff as
  // Edit, but routes to the existing /download-resume page instead —
  // that page mounts a single instance of the template component, which is
  // what its own built-in PDF pipeline (window.__resumePageStarts, etc.)
  // relies on. Triggering a download straight from the dashboard grid isn't
  // safe: every card's template component is mounted at once for its
  // thumbnail, and they'd all race to write those same window globals.
  const handleDownloadResume = (item: ResumeItem) => {
    setLocalStorage("chosenTemplate", enrichTemplateForStorage(item.template));
    setLocalStorage("editingResumeIdAndData", {
      id: item.id,
      resume_data: item.resume_data,
  ...
  // Clears the identifiers left behind by a previous Edit/Download so a
  // genuinely new resume doesn't inherit them. Without this, clicking
  // "Create New Resume" after ever editing or downloading a resume would
  // carry the old "editingResumeIdAndData" (and the "oldRouteNameDashboard"
  // flag that gates it) all the way through choose-template → contact →
  // ... → download-resume, making every one of those pages treat the new
  // resume as if it were that old one.
  const handleStartNewResume = () => {
    removeLocalStorage("editingResumeIdAndData");
    removeLocalStorage("latest_resume_id");
  ...
  // Full-page loader shown while the initial dashboard data is being fetched
  if (isPageLoading) {
    return (
      <ProtectedRoute>
        <div className="min-h-screen bg-gradient-to-br from-indigo-50/30 via-white to-indigo-50/20 flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            <motion.div
              className="w-12 h-12 rounded-full border-4 border-indigo-100 border-t-indigo-600"
              animate={{ rotate: 360 }}
  ...
  return (
    <ProtectedRoute>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            >
              <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-indigo-100 overflow-hidden hover:shadow-md transition-all duration-300 group">
                <div className="relative h-20  bg-gradient-to-br from-indigo-700 to-purple-500">
                  <motion.div
                ...
                <div className="p-4 sm:p-5 md:p-6 pt-10 sm:pt-12">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {[
                      ...
                      >
                        <div
                          className={`p-1.5 sm:p-2 ${item.iconBg} rounded-lg`}
                        >
                          <item.icon
                            className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${item.iconColor}`}
                          />
                        </div>
                  ...
                  <motion.button
                    ...
                    className="w-full mt-3 px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-indigo-600 to-purple-500 text-white font-medium rounded-lg sm:rounded-xl hover:shadow-lg hover:shadow-indigo-500/25 transition-all duration-300 flex items-center justify-center gap-2 group cursor-pointer text-xs sm:text-sm"
                  >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                          ...
                          <div className="text-left sm:text-right">
                            <p className="text-2xl sm:text-3xl font-bold text-white">
                              ₹
                              {usersCurrentPlan?.plan_details?.discount_price ||
                                "0"}
                            </p>
            ...
            >
              {filteredOldResumeData.length > 0 ? (
                filteredOldResumeData.map((item, index) => {
                  ...
                  return (
                    <motion.div
                      // Stable key (resume id) instead of array index — this is
                      // what previously caused the hover/overlay state to "jump"
                      // to the top card after a delete: with an index key, React
                      // reused the DOM node at position 0 for whatever resume
                      // slid into that slot, dragging its hover/animation state
                      // along with it instead of unmounting cleanly.
                      key={item.id}
                      layout
                      ...
                      className={`relative group  ${isPlanExpired ? "opacity-75" : ""}`}
                      style={{
                        height: isMobile
                    ...
                    >
                      ...
                      {/* Per-card delete-in-progress overlay */}
                      {isDeletingThis && (
                        <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex items-center justify-center z-20">
                          <motion.div
                            className="w-8 h-8 rounded-full border-4 border-indigo-100 border-t-indigo-600"
                            animate={{ rotate: 360 }}
                            transition={{
                              duration: 0.8,
                              repeat: Infinity,
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                            <div className="flex items-center justify-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                              ...
                              <motion.button
                                ...
                                className="bg-white rounded-full p-1.5 sm:p-2.5 hover:bg-emerald-50 transition-all duration-300 shadow-lg cursor-pointer group/btn"
                                title="Download resume"
                              >
                                <FiDownload className="h-3 w-3 sm:h-4 sm:w-4 text-emerald-600 group-hover/btn:scale-110 transition-transform" />
                              </motion.button>
                              <motion.button
                                whileHover={{ scale: 1.1 }}
                                whileTap={{ scale: 0.95 }}
                            ...
                            <>
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="w-full px-3 py-2 text-left text-xs text-gray-700 hover:bg-emerald-50 hover:text-emerald-600 transition-colors flex items-center gap-2"
                                >
      ...
      {/* View Resume Modal - Responsive, same-page preview */}
      <AnimatePresence>
        {viewingResume && (
          <div
          ...
          >
            <motion.div
              ...
              transition={{ type: "spring", damping: 25, stiffness: 300 }}
              className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="bg-gradient-to-r from-indigo-600 to-indigo-500 p-4 sm:p-6 flex-shrink-0">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <h2 className="text-base sm:text-lg lg:text-xl font-bold text-white">
                        {viewingResume.name || "Resume Preview"}
                      </h2>
                      <p className="text-indigo-100 text-[11px] sm:text-sm">
                        Template: {viewingResume.template?.id}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <motion.button
                      ...
                      className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-medium rounded-lg transition cursor-pointer"
                    >
              ...
              {/* Scaled A4 preview.
                  IMPORTANT: pass viewMode={true} — templates key off
                  `isThumbnail = !!alldata && !viewMode` to decide how to
                  render. Without viewMode, alldata alone flips them into
                  their tiny 0.36-scale thumbnail mode (built for the grid
                  cards), which is why the resume showed up shrunk into a
                  corner here. viewMode={true} forces the real full-size
                  paginated preview, which we then scale ourselves.
                  We use CSS `zoom` (not `transform: scale`) so the box
                  actually reflows to its scaled size — transform leaves
                  the original unscaled height in the layout and produces
                  a large blank gap below the resume. */}
              <div
                ref={viewContainerRef}
                className="flex-1 overflow-y-auto bg-gray-100 p-3 sm:p-6 flex justify-center"
              >
                <div
                  style={
                    {
                ...
                >
                  {(() => {
                    const ViewComponent = viewingResume.component;
                    return (
                      <ViewComponent
                        alldata={viewingResume.resume_data}
                        viewMode={true}
                      />
                    );
                  })()}
