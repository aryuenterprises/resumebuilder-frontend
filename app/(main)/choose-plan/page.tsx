...
import React, { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { FiStar, FiHeart, FiX, FiCheckCircle, FiArrowRight } from "react-icons/fi";
import { IoSparkles, IoCheckmarkCircle, IoDiamondOutline, IoArrowForward } from "react-icons/io5";
          ...
          >
            ...
            <div className="p-6">
                ...
                <div className="space-y-3">
                  {plan.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start gap-2">
                      ...
                      <span className={`text-sm ${feature.highlight ? "text-gray-900 font-semibold" : feature.included ? "text-gray-700" : "text-gray-400"}`}>
                        {feature.name}
                      </span>
                    </div>
                  ))}
              ...
              <button
                onClick={handlePayment}
                disabled={loading}
                className={`w-full py-3 rounded-xl font-semibold text-white transition-all duration-300 ${loading ? "opacity-60 cursor-not-allowed" : "hover:shadow-lg"} bg-gradient-to-r ${plan.color}`}
              >
...
export default function ChoosePlanPage() {
  ...
  const handleSelectPlan = async (plan: Plan) => {
    ...
    if (plan.price === 0) {
      setLoading(true);
      try {
        const response = await axios.post(`${API_URL}/api/payment-razor/free-plan`, {
          userId: userId,
          planId: plan.id,
        });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan, index) => {
            ...
            return (
              ...
              >
                <div className={`relative h-full bg-white rounded-2xl overflow-hidden transition-all duration-500 ${isHovered ? "scale-105 shadow-2xl" : "shadow-lg"}`}>
                  {/* Popular Badge */}
                  {plan.popular && (
                  ...
                  {/* Card Header */}
                  <div className={`bg-gradient-to-r ${plan.color} p-5 text-white relative overflow-hidden`}>
                    <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -mr-8 -mt-8"></div>
                    <div className="relative">
                      <div className="flex items-center justify-between mb-3">
                        <div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm">{plan.icon}</div>
                        {plan.badge && !isSamePlan && (
                          <span className="px-2 py-1 bg-white/20 rounded-full text-xs backdrop-blur-sm">{plan.badge}</span>
                        )}
                        {isSamePlan && (
                          <span className="px-2 py-1 bg-emerald-500/80 text-white text-xs font-semibold rounded-full flex items-center gap-1">
                            <FiCheckCircle className="w-3 h-3" />
                            Active
                          </span>
                        )}
                      ...
                      <p className="text-white/80 text-xs mb-3">{plan.description}</p>
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-bold">₹{plan.price}</span>
                        <span className="text-white/80 text-xs">/{plan.interval}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5">
                    <ul className="space-y-2 mb-5">
                      {plan.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-2">
                          ...
                          <span className={`text-xs ${feature.highlight ? "text-gray-900 font-semibold" : feature.included ? "text-gray-700" : "text-gray-400"}`}>
                            {feature.name}
                          </span>
                        </li>
                      ))}
                    ...
                    {isSamePlan ? (
                      <button disabled className="w-full py-2.5 bg-emerald-50 text-emerald-700 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 cursor-default border border-emerald-200">
                        <FiCheckCircle className="w-4 h-4" />
                        Current Plan
                      </button>
                    ) : (
                      <button
                        ...
                        disabled={loading}
                        className={`w-full py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 ${plan.price === 0 ? "bg-gray-100 text-gray-700 hover:bg-gray-200" : `bg-gradient-to-r ${plan.color} text-white shadow-md hover:shadow-lg`} ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                      >
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="p-4 text-left text-sm font-semibold text-gray-900">Features</th>
                  {plans.map((plan) => (
              ...
              <tbody>
                {comparisonFeatures.map((feature, idx) => (
                  <tr key={idx} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="p-4 text-center text-sm text-gray-600">{feature.pro}</td>
                    <td className="p-4 text-center text-sm text-gray-600">{feature.proPlus}</td>
        ...
        <div className="mt-16 text-center">
          <p className="text-xs text-gray-400 mb-4">Trusted by professionals from</p>
          <div className="flex flex-wrap justify-center items-center gap-6 opacity-40">
            ...
            <span className="text-sm font-semibold text-gray-500">Amazon</span>
            <span className="text-sm font-semibold text-gray-500">Meta</span>
            <span className="text-sm font-semibold text-gray-500">Apple</span>
          </div>
        </div>
      </div>
