  ...
  const canUserSelectFreePlan = (
    ...
    if (!currentPlan) {
      return {
        ...
        friendlyMessage:
          "You already have access to the Free plan. It was automatically activated when you created your account.",
        reason:
          "Free plan is automatically activated on registration. You already have access to it.",
      };
    }
    ...
    if (planLower === "free") {
      return {
        ...
        friendlyMessage:
          "You're currently enjoying the Free plan. Check out our Pro and Premium plans for more features!",
    ...
    if (isExpired) {
      return {
        ...
        friendlyMessage:
          "Your Pro/Premium subscription has expired. To continue using our services, please renew your subscription or upgrade to a new plan.",
        reason:
          "Your subscription has expired. Please subscribe to Pro or Premium to continue using our services.",
      };
    }
    ...
    return {
      ...
      friendlyMessage:
        "The Free plan is only available for new users. You already have an active subscription. Explore our Pro and Premium plans for more features!",
      reason:
        "Free plan is not available for your current subscription status.",
    };
  };
  ...
  const handleSelectPlan = async (plan: Plan) => {
    ...
    if (plan.regularPrice === "0" || plan.name.toLowerCase() === "free") {
      ...
      try {
        ...
        const { allowed, reason, friendlyMessage, title } =
          canUserSelectFreePlan(currentPlan, isExpired, true);
        ...
        if (!allowed) {
          ...
          showModal("info", {
            ...
            message:
              friendlyMessage ||
              reason ||
              "Free plan is not available for your account.",
        ...
        {planDetails.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5 md:gap-6 items-stretch">
            {[...planDetails].reverse().map((plan, index) => {
              ...
              const isFreePlan =
                plan.regularPrice === "0" || plan.name.toLowerCase() === "free";
              const { allowed: canSelectFree } = canUserSelectFreePlan(
                usersCurrentPlan,
                isSubscriptionExpired,
                isAuthenticated,
              );
              ...
              const isFreePlanActive =
                isAuthenticated &&
                isFreePlan &&
                usersCurrentPlan?.toLowerCase() === "free";
                      ...
                      {isSamePlan || isFreePlanActive ? (
                        ...
                        >
                          ...
                          {isSubscriptionExpired
                            ? "Expired - Renew Now"
                            : usersCurrentPlan &&
                                ["pro", "premium", "pro plus"].includes(
                                  usersCurrentPlan.toLowerCase(),
                                )
