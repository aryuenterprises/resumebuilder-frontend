...
//   const [loading, setLoading] = useState<boolean>(false);
//   const [focusedField, setFocusedField] = useState<string | null>(null);
//   const [showGeneratedNotification, setShowGeneratedNotification] = useState<boolean>(false);
//   const [passwordStrength, setPasswordStrength] = useState<PasswordStrength>({
//     score: 0,
//     label: "Very Weak",
//     color: "bg-red-500",
...
//     toast.className = 'fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg text-sm z-50 animate-fade-in-up';
...
const ChangePassword = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!password.trim()) {
      showModal('warning', {
        ...
        subMessage: "Choose a strong password for better security.",
        email: ""
      });
      return false;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 flex flex-col">
      ...
      {/* Generated Password Notification Toast */}
      <AnimatePresence>
        {showGeneratedNotification && (
          ...
          >
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 py-2 rounded-lg shadow-lg text-sm flex items-center gap-2">
              <FiCheckCircle className="w-4 h-4" />
              <span>Password generated and copied to clipboard!</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
