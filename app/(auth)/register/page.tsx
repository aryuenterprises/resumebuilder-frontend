...
export default function RegisterForm() {
  ...
  const validateForm = (): boolean => {
    ...
    // Must start with a letter, 2–100 chars. Rejects digits, <, >, etc.
    const PLACE_RE = /^[\p{L}][\p{L}\s'.,()\-\/]{1,99}$/u;
    ...
    // ---------- First / Last name ----------
    const firstName =
      typeof values.firstName === "string" ? values.firstName.trim() : "";
    if (!firstName) newErrors.firstName = "First name is required";
    else if (!NAME_RE.test(firstName))
      newErrors.firstName = "First name contains invalid characters";

    const lastName =
      typeof values.lastName === "string" ? values.lastName.trim() : "";
    ...
    if (!state) {
      ...
      newErrors.state = "State must be at most 100 characters";
    ...
    // ---------- Country ----------
    const country =
      typeof values.country === "string" ? values.country.trim() : "";
    if (!country) {
      ...
      !PLACE_RE.test(country) &&
      !ISO2_RE.test(country.toUpperCase()) &&
      !ISO3_RE.test(country.toUpperCase())
    ) {
    ...
    if (!password.trim()) {
      ...
      const isCommon = commonWeak.some((w) => lowered.includes(w));
  ...
  const handleGoogleError = () => {
    setErrorMessage("Google Sign-up was cancelled or encountered an error.");
    setShowErrorModal(true);
  };
