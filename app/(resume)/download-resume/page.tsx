...
//   const handleDragEnter = (i: number) => { dragOverItem.current = i; setDragOver(i); };
//   const handleDragEnd   = () => {
//     if (dragItem.current !== null && dragOverItem.current !== null) {
//       const next = [...order];
...
//   const moveUp   = (i: number) => { if (i === 0) return; const n = [...order]; [n[i-1], n[i]] = [n[i], n[i-1]]; onChange(n); };
//   const moveDown = (i: number) => { if (i === order.length-1) return; const n = [...order]; [n[i], n[i+1]] = [n[i+1], n[i]]; onChange(n); };
...
//           <span className="text-base">👤</span>
//           <span className="text-sm font-medium text-gray-600 flex-1">Contact Header</span>
//           <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded-full font-medium">Locked</span>
//           <svg className="w-4 h-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
//           </svg>
//         </div>
...
//           <span className="text-xs font-medium text-gray-700 flex-1">{SECTION_LABELS[section]}</span>
...
//   <div>
//     <p className="text-xs text-gray-400 mb-3">Drag sections to reorder — header is always first</p>
//     <DraggableSectionList
//       order={customization.sectionOrder}
//       onChange={(sectionOrder) => onChange({ ...customization, sectionOrder })}
//     />
//     <p className="mt-3 text-xs text-gray-400 text-center">💡 Put your strongest section near the top</p>
//   </div>
// );
...
//     const isDropTarget =
//       dropTarget?.col === col && dropTarget?.index === index;
//     const isCrossTarget =
//       isDropTarget && draggingItem?.col !== col;
...
//                 d={col === "left" ? "M13 5l7 7-7 7M5 5l7 7-7 7" : "M11 19l-7-7 7-7m8 14l-7-7 7-7"} />
//             </svg>
//           </button>
//         </div>
//       </div>
...
//           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
//         </svg>
//         <span className="text-xs font-semibold text-indigo-600 flex-1 text-center">Header</span>
//         <span className="text-xs text-indigo-400 bg-indigo-200 px-1.5 py-0.5 rounded-full font-medium">Locked</span>
//       </div>
...
//             <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Left</span>
//           </div>
//           {left.length === 0
...
//             <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Right</span>
//           </div>
//           {right.length === 0
...
//             </span>
//           )}
//           <span style={{ fontFamily: font.value }} className="text-2xl font-bold text-gray-800 leading-none mb-1">{font.preview}</span>
//           <span className="text-xs text-gray-500 font-medium leading-tight">{font.label}</span>
//         </button>
//       ))}
//     </div>
...
//   isDownloading: boolean;
//   isTwoColumn: boolean;
// }> = ({ customization, onChange, onReset, showDownload, onDownload, isDownloading, isTwoColumn }) => {
//   const [activeTab, setActiveTab] = useState<"font" | "sections">("font");
...
//         <div className="flex items-center justify-between mb-1">
//           <h2 className="text-xs sm:text-sm font-semibold text-gray-800 tracking-wide uppercase">Customize</h2>
//           <button onClick={onReset} className="text-xs text-gray-400 hover:text-gray-600 transition-colors underline-offset-2 hover:underline">
//             Reset
//           </button>
//         </div>
//         <p className="text-xs text-gray-400">Personalize your resume before download</p>
...
//       {/* Tabs */}
//       <div className="flex border-b border-gray-100">
//         <button onClick={() => setActiveTab("font")}
...
//           <div>
//             <p className="text-xs text-gray-400 mb-3">Choose a font that matches your style</p>
//             <FontPicker value={customization.fontFamily} onChange={(fontFamily) => onChange({ ...customization, fontFamily })} />
//           </div>
//         )}
//         {activeTab === "sections" && (
//           isTwoColumn
//             ? <TwoColumnSections customization={customization} onChange={onChange} />
//             : <SingleColumnSections customization={customization} onChange={onChange} />
//         )}
//       </div>
...
//     const chosenTemplate = getLocalStorage<Template>("chosenTemplate");
//     const selectedResume = templateData.find(
//       (r) => r.id == chosenTemplate?.id || r.id == chosenTemplate?.templateId,
//     );
//     setSelectedComponent(() => selectedResume?.component ?? null);
...
//     const TWO_COLUMN_NAME_PATTERNS = /templatetwo|template_two|template-two|twocolumn/i;
...
//     if (isTwo) {
//       setCustomization((prev) => ({ ...prev, fontFamily: "'Nunito', sans-serif" }));
//     }
//   }, []);
...
//       fontFamily: isTwoColumn ? "'Nunito', sans-serif" : "'Poppins', sans-serif",
//       twoColumnOrder: {
//         left:  [...DEFAULT_TWO_COLUMN_ORDER.left],
//         right: [...DEFAULT_TWO_COLUMN_ORDER.right],
//       },
...
//           <div className="text-center">
//             <p className="text-gray-500 mb-2">No resume template selected.</p>
//             <Link href="/dashboard" className="text-emerald-500 hover:underline font-medium">← Go to Dashboard</Link>
//           </div>
//         </div>
//         <Footer />
//       </ProtectedRoute>
...
//           <div className="mb-4 sm:mb-6">
//             <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Finalize Your Resume</h1>
//             <p className="text-xs sm:text-sm text-gray-500 mt-1">Customize style &amp; layout, then download your polished PDF.</p>
//           </div>
...
//               <div className="block lg:hidden">
//                 <CustomizationPanelInner
//                   customization={customization} onChange={setCustomization}
//                   onReset={handleReset} showDownload={false}
...
//                 <CustomizationPanelInner
//                   customization={customization} onChange={setCustomization}
//                   onReset={handleReset} showDownload={true}
//                   onDownload={handleDownload} isDownloading={isDownloading}
//                   isTwoColumn={isTwoColumn}
//                 />
//               </div>
...
//                   <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-full border">A4 · PDF ready</span>
//                 </div>
//                 <div ref={containerRef} className="flex justify-center overflow-hidden">
//                   <div style={{ transform: `scale(${previewScale})`, transformOrigin: "top center", width: `${794 / previewScale}px` }}>
//                     {React.createElement(selectedComponent, { customization })}
//                   </div>
//                 </div>
//               </div>
