...
//                                       e.g., Highlight scalability features,
...
const ProjectsForm = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-linear-to-br from-slate-50 via-white to-indigo-50/40">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-3 sm:p-6 lg:p-8">
              {projects?.map((project: Project, index: number) => (
                <div
                  key={project.id}
                  className={`bg-white rounded-xl sm:rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 mb-3 sm:mb-4 
                    ${draggedItemId === project.id ? "opacity-50" : ""}
                    ${dragOverItemId === project.id ? "border-2 border-indigo-400 shadow-lg transform scale-[1.02]" : ""}
                  `}
                >
                  ...
                  >
                    <div className="p-3.5 sm:p-5 space-y-3.5 sm:space-y-5 border-t border-gray-100">
                      ...
                      {/* Tech Stack */}
                      <div>
                        <label className="block text-[10px] sm:text-xs font-semibold tracking-wider text-gray-500 mb-1 sm:mb-2">
                          TECH STACK{" "}
                          <span className="text-[10px] sm:text-[11px] text-gray-400 font-medium tracking-normal">
                            (Press Enter to add tech stack)
                          </span>
                        </label>
                        <div className="flex gap-2 mb-2.5 sm:mb-3">
                          <input
                            ...
                            className="flex-1 px-3 sm:px-4 py-2 sm:py-3 bg-white border-2 border-gray-200 rounded-lg sm:rounded-xl text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all"
                            placeholder="React, Node.js, MongoDB"
                          />
                          <button
                            onClick={() => addTechStack(project.id)}
                            className="px-3 sm:px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-indigo-100 hover:text-indigo-600 transition-colors text-xs sm:text-sm font-medium"
                          >
                            Add
                          </button>
                        </div>
                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                          {project.techStack.map((tech: string) => (
                            <span
                              key={tech}
                              className="inline-flex font-medium items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 bg-indigo-50 text-indigo-700 text-[11px] sm:text-xs rounded-lg"
                            >
                              {tech}
                              <button
                                onClick={() =>
      ...
      {/* AI Response Popup */}
      {showPopup && Airesponse && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-500 p-3 sm:p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden"
          ...
          <div className="flex justify-between items-center gap-2 sm:gap-4">
            ...
            >
              ...
              <div className="relative flex items-center justify-center gap-1.5 sm:gap-2">
                <span className="hidden xs:inline">Continue to Summary</span>
