...
export default function RootLayout({
  ...
  //   // NEW: user just came from the resume-upload flow — the upload already
  //   // populated the context data, so skip the "Continue vs New" modal entirely.
  //   if (isUploadMode) {
  ...
  // }, [pathname, userId, isUploadMode]); // ⬅️ add isUploadMode to deps
  ...
  useEffect(() => {
    ...
    // ============================================================
    // FIX: Turn off loader immediately if we don't need to check
    // ============================================================
    if (!isContactPage || !userId) {
      setIsLoading(false);
      return;
    }

    if (isOldRouteNameDashboard) {
      setIsLoading(false); // Ensure loading is off if route is handled
      return;
    }

    if (isUploadMode) {
      setSessionStorage("resumeChoiceChecked", "true");
      setIsLoading(false); // Ensure loading is off if in upload mode
      return;
    }

    const alreadyChecked = getSessionStorage("resumeChoiceChecked");
    if (alreadyChecked) {
      setIsLoading(false); // Ensure loading is off if already checked
      return;
    }

    // Only run the API check if we passed all the guards above
    const checkExisting = async () => {
      try {
        setIsLoading(true); // Start loading specifically for the API call
        const response = await apiClient.get(`/user-resumes`);
        const hasExisting =
          Array.isArray(response.data) && response.data.length > 0;
